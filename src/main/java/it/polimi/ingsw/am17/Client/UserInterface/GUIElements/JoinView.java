...
public class JoinView {
    ...
    private void buildUI() {
        ...
        title.setStyle("-fx-text-fill: white; -fx-font-size: %dpx; -fx-font-weight: bold;"
                .formatted(ScreenScale.sizeInt(36)));
        ...
        idField.setStyle("-fx-font-size: %dpx; -fx-alignment: center;".formatted(ScreenScale.sizeInt(18)));

        // Join button
        Button joinBtn = getJoinBtn(idField);

        Button backBtn = new Button("BACK");
        backBtn.setPrefWidth(ScreenScale.size(400));
        backBtn.setCursor(Cursor.HAND);
        backBtn.setStyle("-fx-font-weight: bold; -fx-font-size: %dpx; -fx-padding: %dpx %dpx;".formatted(
        ...
        gameList.setMinHeight(ScreenScale.size(150));
        gameList.setPrefHeight(ScreenScale.size(150));
        gameList.setPrefWidth(ScreenScale.size(400));
        gameList.setMaxWidth(ScreenScale.size(400));
        gameList.setStyle("-fx-font-size: %dpx;".formatted(ScreenScale.sizeInt(16)));
        ...
        gameList.getSelectionModel().selectedItemProperty().addListener((_, _, newValue) -> {
            if (newValue != null) {
                ...
                idField.setStyle("-fx-font-size: %dpx; -fx-alignment: center;".formatted(ScreenScale.sizeInt(18))); // Reset border in case of previous error
            }
        });
    ...
    private Button getJoinBtn(TextField idField) {
        ...
        joinBtn.setStyle("-fx-background-color: #27ae60; -fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: %dpx; -fx-padding: %dpx %dpx;".formatted(
                ScreenScale.sizeInt(18),
                ScreenScale.sizeInt(12),
                ScreenScale.sizeInt(20)
        ));
        ...
        joinBtn.setOnAction(_ -> {
            ...
            if (!gameID.isEmpty()) {
                try {
                    ...
                    idField.setStyle("-fx-border-color: red; -fx-border-width: %dpx; -fx-font-size: %dpx; -fx-alignment: center;".formatted(
